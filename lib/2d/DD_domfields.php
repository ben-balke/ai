...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/lib/2d/DD_domfields.php,v 1.2 2016/03/16 18:47:00 secwind Exp $
*************************************************************************/

require_once "2d/DD_sql.php";
require_once "2d/DD_util.php";
...
function DD_MakeField ($array, $extension = null)
{
	...
	case "select":
		...
		break;
	case "checkbox":
		$field = new DD_DomCheckBox ($array, $extension); // Done
		break;
	case "text":
		$field = new DD_DomText ($array, $extension); // Done
		break;
	case "password":
		$field = new DD_DomPassword ($array, $extension); // Done
		break;
	case "button":
		$field = new DD_DomButton ($array, $extension); // Done
		break;
	case "file":
		$field = new DD_DomFile ($array, $extension);
		break;
	case "hidden":
		$field = new DD_DomHidden ($array, $extension); // Done
		break;
	case "textarea":
		$field = new DD_DomTextArea ($array, $extension); // Done
		break;
	case "submit":
		$field = new DD_DomSubmit ($array, $extension); // Done
		break;
	case "multiitem":
		$field = new DD_DomMultiItem ($array, $extension);
		break;
	case "radio":
		$field = new DD_DomRadio ($array, $extension); // Done
		break;
	case "tab":
	...
	case "links":
		...
		break;
	case "work":
		$field = new DD_DomWork ($array, $extension); // Done
		break;
	case "display":
		$field = new DD_DomDisplay ($array, $extension); // Done
		break;
	case "image":
...
class DD_ListSource
{
	...
	function __construct ($cache, $parse)
	{
		$this->m_cache = $cache;
		$this->m_parse = $parse;
	}
...
{
	function __construct ($low, $high, $step = 1)
	{
...
{
	var			$m_origArray;
	function __construct ($array, $cache = true, $parse = false)
	{
...
{
	var			$m_origArray;
	function __construct ($directory, $regex)
	{
		parent::DD_ListSource (true, false);
		$this->m_array = array();
...
class DD_SqlListSource extends DD_ListSource
{
	...
	function __construct ($connect, $query, $cache = true, $parse = false)
	{
		parent::DD_ListSource ($cache, $parse);
		$this->m_query = $query;
		$this->m_connect = $connect;
...
class DD_DomField
{
	var			$m_class;		// add class fields to dom item.
	var			$m_name;		// the tag parser name and how it is id'ed.
	var			$m_value;		// value of the field.
	var			$m_html;		// Addition attributes on the field
	var			$m_readonly;	// Read only attribute
	var			$m_default;		// The default value.  This is set on prepareForRecord.
	var			$m_keysource;	// Before an inset is executed privides value.
	var			$m_datatype;	// low level datatype
	var			$m_format;		// format for the above datatype
	...
	var			$m_formlayout; // Field containing 3 comma separated numbers
	var			$m_help = ""; // Also used for control.  Use a blank string so validation can show.
	// <offset>,<label>,<field>
	// offset modifies the style col-sm-offset-<offset> 
	// lebel modifies the style col-sm-offset-<offset> 
	var			$m_required = false;  // Required identifier
		/*
	...
	function setState (&$array)
	{
		...
		if (isset ($array ['required']))
			$this->m_required = $array ['required'];
		if (isset ($array ['help']))
	...
	{
		return ' aria-describedby="help' . $this->getFullName() . '" ';
	}
	function getHelpTag ($tagParser)
	{
		return '<span id="help' . $this->getFullName () . '" class="help-block with-errors">' . $tagParser->parseContent ($this->m_help) . '</span>';
	}
	function getLabelTag ($tagParser)
	{
		$text =	"";
		if (isset ($this->m_label))
		{
			$text .= '<label for="' .  $this->getFullName () . '" class="control-label">' . $tagParser->parseContent ($this->m_label) . '</label>';
		}
		return $text;
	}
	function getLabel()
	...
	public function getAttributeContent ($attribute, $tagParser)
	{
		...
		{
		case 'label':
			return $this->getLabelTag ($tagParser);
		default:
			break;
		}
		return null;
...
class DD_DomSelect extends DD_DomField
{
	...
	function makeHeader ($tagParser)
	{
		...
		$text .= '<select class="form-control" title="pick something" name="' . $this->getFullName () . $nameext . '" ' . $this->getHtml ($tagParser) . ' id="' . $this->getFullName () .  '"';
		if (isset ($this->m_multi))
		{
			$text .= ' multiple ';
		}
		...
		if ($this->m_required)
		{
			if (isset ($this->m_noselection))
				$text .= ' data-noselect0="noselect0"';
			else
				$text .= ' data-noselect="noselect"';
		}
		$text .= $this->getHelpLink () . '>';
		if (isset ($this->m_noselection))
		{
	...
	function toString ($tagParser)
	{
		...
		$text .= '</select></div>' . $this->getHelpTag ($tagParser) . '</div>';
		return $text;
	}
}
class DD_DomRadio extends DD_DomField
{
	...
	function toString ($tagParser)
	{
		...
		if (isset ($this->m_label))
		{
			$text .= $this->getLabelTag ($tagParser) . '<div class="input-group">';
		}
		...
		if (isset ($this->m_label))
			$text .= "</div>";
		$text .= $this->getHelpTag ($tagParser);
		$text .= "</div>";
		return $text;
	}
} 
...
class DD_DomText extends DD_DomField
{
	...
	var				$m_placeholder;
	var				$m_maxlength;
	var				$m_minlength; // See Bootstrap 3 Validator
	var				$m_pattern;	  // See Bootstrap 3 Validator
	var				$m_size;
	...
	var				$m_addon = false;
	var				$m_needsFeedback = false;
	var				$m_hasRightAddon = false;
	...
	function __construct ($array, $extension = null)
	{
		...
		if (isset ($extension))
			$this->setState ($extension);
		if (!isset ($this->m_inputtype))
		{
			$this->m_inputtype = 'text';
		}
		if (!$this->m_placeholder)
		{
		 	$this->m_placeholder = $this->m_label;
		}
	}
	...
	function setState (&$array)
	{
		...
		if (isset ($array ['help'])) // Handled by parent.
		{
			$this->m_addon = true;
		}
		...
		{
			$this->m_minlength = $array ['minlength'];
			$this->m_needsFeedback = true;
		}
		if (isset ($array ['pattern']))
		{
			$this->m_pattern = $array ['pattern'];
			$this->m_needsFeedback = true;
		}
	}
	...
	function toString ($tagParser)
	{
		...
		$extraClasses = "";
		if ($this->m_needsFeedback && $this->m_hasRightAddon)
		{
			//die ("field " . $this->getFullName () . " has both a right addon and minlength or pattern");
			//TODO: if you have feedback and right addon it causes problems 
			//$extraClasses = " has-addon-feedback has-success";
		}
		$text =	'<div class="form-group' . $extraClasses . '">';
		$text .= $this->getLabelTag ($tagParser);
		$text .= '<div class="input-group">';
		...
		$text .= '<input type="' . $this->m_inputtype . '" class="form-control" id="' . $this->getFullName() . '" name="' . $this->getFullName() . '"';
		...
		if (isset ($this->m_minlength))
			$text .= ' data-minlength="' . $this->m_minlength . '"';
		$text .= $this->getHtml ($tagParser);
		...
		//
		// If we have a data validation that can show feed back then
		// add it.  This occurs when we have a pattern or minlength for example.
		//
		if ($this->m_needsFeedback)
		{
			$text .= '<span class="glyphicon form-control-feedback" aria-hidden="true"></span>';
		}
		else if ($this->m_addon == true)
		{
			...
			if (isset ($this->m_help) && isset ($this->m_help_popover))
			{
				$popover = new DD_PopOver (array (
				    'name'=>'popover' //'help_' . $this->getFullName ()
					,'activator'=>'link'
				    ,'content'=>$this->m_help
...
class DD_DomPassword extends DD_DomText
{
	function __construct ($array, $extension)
	{
		parent::__construct($array, $extension);
		$this->m_inputtype = 'password';
	}
}
...
class DD_DomButton extends DD_DomField
{
	...
	function toString ($tagParser)
	{
		if (!isset ($this->m_class))
		{
			$this->m_class = "btn-default";
		}

		$text =  '<button type="button" class="btn ' . $this->m_class . '" name="' . $this->getFullName () . '" id="' . 
			$this->getFullName () . '"';
		$text .= $this->getHtml ($tagParser) . '>';
		$text .= $tagParser->parseContent($this->m_value) . '</button>'; 
		return $text;
	}
}
class DD_DomImage extends DD_DomField
{
	...
	function DD_DomField ($array, $extension) // TODO: This is for legacy derivations.  Please keep around.
	{
		__construct ($array, $extension);
	}
	function __construct ($array, $extension)
	{
		parent::__construct ($array, $extension);
...
class DD_DomCheckBox extends DD_DomField
{
	...
	function toString ($tagParser)
	{
		$text = '<div class="checkbox' . ($this->m_inline ? '-inline' : '') . '"><label>';
		...
		if ($this->m_label)
		{
			$text .= $tagParser->parseContent ($this->m_label);
		}
		$text .= '</label></div>';
...
class DD_DomTextArea extends DD_DomField
{
	...
	function toString ($tagParser)
	{
		...
		$text .=  '<textarea class="form-control" name="' . $this->getFullName() . 
			'" id="' . $this->getFullName () . '"';
...
class DD_DomSubmit extends DD_DomField
{
	...
	function toString ($tagParser)
	{
		...
		$text =  '<button type="submit" class="btn ' . $this->m_class . '" name="' . $this->getFullName () . '" id="' . 
			$this->getFullName () . '"';
		$text .= $this->getHtml ($tagParser) . ' />';
		$text .= $tagParser->parseContent($this->m_value);
