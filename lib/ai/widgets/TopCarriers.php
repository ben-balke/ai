...
/* Color Widget
 */
class TopCarriers extends DynamicWidget 
{
	private			$m_cols;
	function __construct ($name, $cols = 3)
	{
		parent::Widget ($name, "topcarriers", "Top Carriers", "600px"); //, "400px");
		$m_cols = $cols;

	}

	function addCarriers ($tagParser)
	{
...
<tr>
	<th >#</th>
	<th align="left">Top {dd:user.n_top_carriers} Carriers</th>
	<th >Premium</th>
	<th >Revenue</th>
	<th >% Comm</th>
	<th >code</th>
...
<tfoot>
<tr class="">
	<td colspan="2"><a href="{dd:dd.path}querybyins.php">See All Carriers</a></td>
	<td align="right">{dd:tc.premium}&nbsp;</td>
	<td align="right">{dd:tc.revenue}&nbsp;</td>
	<td align="right">{dd:tc.percent}&nbsp;</td>
	<td align="right">&nbsp;</td>
...
CONTENT;
		...
		$select = "select
			...
			from
				cd_policy p
				left outer join cd_insuror i on (i.office_id = p.office_id and 
					i.id = p.insuror_id)
			where
			...
			order by {SQL:user.sort_top_carriers} 
			limit {user.n_top_carriers};";
	...
	function toString ($tagParser)
	{
		...
		$text .= $this->addCarriers ($tagParser);
