...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/lib/2d/DD_valueprovidermodel.php,v 1.1 2016/02/16 21:18:00 secwind Exp $
*************************************************************************/

require_once '2d/DD_tagparser.php';
require_once '2d/DD_model.php';

class DD_ValueProviderModel extends DD_Model
{
	var			$m_update = false;
	var			$m_target;
	var			$m_vp = null;
    ...
    function setState (&$array)
    {
        if (isset ($array ['target']))
			...
			die ("ValueProviderModel must have a target value provider");
		if (!strcmp ($this->m_target, $this->m_name))
			die ("ValueProviderModel target cannot be the same as name");
	...
	function query ($tagParser = null)
	{
		if (!isset ($tagParser))
		{
			die ("DD_ValueProviderModel::query(): must have a tagParser provided");
		}
		$this->m_vp = $tagParser->getValueProvider ($this->m_target);
		if (!isset ($this->m_vp))
		{
			die ("DD_ValueProviderModel::query(): target value provider {$this->m_target} does not exist.");
		}
	}
	function nextRecord ()
	{
	...
	function insertRecord ($tagParser)
	{
		...
		if (!isset ($this->m_vp))
		{
			die ("DD_ValueProviderModel::insertRecord(): target value provider {$this->m_target} does not exist.");
		}
