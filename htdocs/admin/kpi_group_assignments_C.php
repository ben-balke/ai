...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/htdocs/admin/kpi_group_assignments_C.php,v 1.2 2010/10/18 18:54:03 secwind Exp $
*************************************************************************/
...
function selectWithText (str)
{
	...
	for (i = 1; i <= rcds; i++)
	{
		if ($('covname' + i).innerHTML.indexOf (text) != -1)
		{
...
<tr><td class="tag" align=right>Select By Coverage:</td><td>{dd:page.selecttext} {dd:page.selecttextbut}</td></tr>
...
<td nowrap id="coverage_id{dd:rhmap.dd_recordno}">{dd:rhmap.covcode}{dd:rhmap.coverage_id} </td>
<td nowrap id="covname{dd:rhmap.dd_recordno}">{dd:rhmap.covname}</td>
<td nowrap>
...
select 
	...
	,g.code as code 
from cd_coverage c
	left outer join ai_kpi_map_coverage m on (m.office_id = {http.office_id} and m.coverage_id = c.id)
	left outer join ai_kpi_group g on (g.id = m.kpi_group)
where 
	c.office_id = {http.office_id}
order by covname
SQL;
$insert = <<<SQL
	insert into ai_kpi_map_coverage (
		office_id
		,coverage_id
		,kpi_group
	) values (
...
$rhclear = new DD_SqlRecordHandler (array (
	'name'=>'rhclear'
	,'connect'=>'sql.ai'
	,'insert'=>"delete from ai_kpi_map_coverage where office_id = {http.office_id}"
));
...
$rhmap->makeField (array ('name'=>'coverage_id','type'=>'hidden','multirecord'=>true));
$rhmap->makeField (array ('name'=>'covcode'));
$rhmap->makeField (array ('name'=>'covname'));
