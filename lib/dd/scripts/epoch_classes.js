...
//-----------------------------------------------------------------------------
Epoch.prototype.setDateFromTarget = function () //PRIVATE: initialize calendar date from target field
{
	var		startDate;
	startDate = new Date ();
	if (this.tgt.value != '')
	{
		var dateStr = this.tgt.value;
		var month;
		var day;
		var year;
		//
		...
		month = parseInt (stripLeadingZeros (matchArray[1])); // parse date into variables
		day = parseInt (stripLeadingZeros (matchArray[3]));
	...
	this.selectedDates = new Array(startDate);
	this.goToMonth(startDate.getFullYear (),startDate.getMonth ());
...
Epoch.prototype.setLang = function()  //all language settings for Epoch are made here.  Check Date.dateFormat() for the Date object's language settings
{
	...
	this.closebtn_title = 'Closes the calendar without setting the date';
...
//-----------------------------------------------------------------------------
Epoch.prototype.closeSelections = function ()  //PRIVATE: reset the calendar's selection variables to defaults
{
	if(this.mode == 'popup') {//hide the calendar if in popup mode
		this.hide();
	}
