...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/htdocs/customerlist.php,v 1.4 2012/08/28 21:13:55 secwind Exp $
*************************************************************************/
...
<thead>
<caption>
    {dd:-user.export}<div style="float:right;"><a href="{dd:dd.path}customerlist.php/customerlist.xls?xml=true{dd:page.criteria}">Export</a>&nbsp;<a href="{dd:dd.path}customerlist.php/customerlist.xls?xml=true{dd:page.criteria}"><img border=0 src="{dd:dd.images}excel.gif" alt="Export to Microsoft Excel"></a></div>
Customers
</caption>
</thead>
<tr><td colspan="8">
...
<tr>
	<th >#</th>
	<th ><a href="#" onclick="sortOption ('cust_name')">Name</a></th>
	<th ><a href="#" onclick="sortOption ('cust_premium desc')">Premium</a></th>
	<th ><a href="#" onclick="sortOption ('cust_revenue desc')">Revenue</a></th>
	<th ><a href="#" onclick="sortOption ('cust_percent desc')">% Comm</a></th>
	<th ><a href="#" onclick="sortOption ('policy_count desc, cust_revenue desc')">Policies</a></th>
	<th ><a href="#" onclick="sortOption ('claim_count desc')">Claims</a></th>
	<th ><a href="#" onclick="sortOption ('cust_code')">Code</a></th>
</tr>
CONTENT;
...
<tr class="{dd:rh.dd_oddeven}">
	...
	<td valign="top" ><a href="{dd:dd.path}customer.php?customer_id={dd:~rh.cust_id}">{dd:rh.cust_name}</a>&nbsp;</td>
	<td valign="top" align=right>{dd:rh.cust_premium}&nbsp;</td>
	<td valign="top" align=right>{dd:rh.cust_revenue}&nbsp;</td>
	<td valign="top" align=right>{dd:rh.cust_percent}&nbsp;</td>
	<td valign="top" align=right nowrap>{dd:rh.policy_count}&nbsp;</td>
	<td valign="top" align=right nowrap>{dd:rh.claim_count}&nbsp;</td>
	<td valign="top" align=left nowrap>{dd:rh.cust_code}&nbsp;</td>
</tr>
CONTENT;
...
<tr class="">
	...
	<td align=right>{dd:rh.cust_premium}&nbsp;</td>
	<td align=right>{dd:rh.cust_revenue}&nbsp;</td>
	<td align=right>{dd:rh.cust_percent}&nbsp;</td>
	<td align=right>{dd:rh.policy_count}&nbsp;</td>
	<td align=right>{dd:rh.claim_count}&nbsp;</td>
...
************************************/
$page = new AI_page ("page", "CustomerList");
...
{
	$select = "select
		distinct p.customer_id as cust_id
		, c.name1 as cust_name
		, c.searchname as cust_code
	...
	from
		cd_transaction p
		left outer join cd_customer c on (c.office_id = p.office_id and c.id = p.customer_id)
	where
		...
		. $criteriaBuilder->getWhere () . " group by cust_id, cust_name, cust_code order by {SQL:user.sort_customers}
		{SQL:page.pagelimit}";
...
if (isset ($_GET ['xml']))
{
	...
	if (isset ($_GET ['fullcust']))
	{
		$select = "select * from cd_customer where id in (select
				distinct p.customer_id as cust_id
			from
				cd_policy p
			where
			...
			and p.book_start <= date_trunc('day',now()))
			and p.premium is not null  and p.premium != 0"
			. $criteriaBuilder->getWhere () . ") order by searchname";
	}
	$xmlFactory->generateRecords ("customer", $select);
	// BBB this uses lots of memory if we cache it.
...
$pager = new PagerField (array (
		'name'=>'pager',
		'pagesize'=>'{dd:-http.pagesize|user.n_customers|=25}',
		'curpage'=>'{dd:-http.page}',
		'url'=>'customerlist.php?searchas={dd:%@http.searchas}'.$criteriaBuilder->getCriteria ().'&action={dd:@http.action}',
		'connect'=>'sql.ai',
		'countsql'=>$countsql
		));
$page->addField ($pager);
