...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/lib/dd/DD_recordhandlers.php,v 1.8 2010/11/12 08:29:27 secwind Exp $
*************************************************************************/
...
class DD_FieldValueProvider extends DD_ValueProvider
{
	...
	function DD_FieldValueProvider ($name, $autofields = false)
	{
		parent::DD_ValueProvider ($name);
 ...
 */
class DD_RecordHandler extends DD_FieldValueProvider
{
	...
	function DD_RecordHandler ($name, $autofields)
	{
		parent::DD_FieldValueProvider ($name, $autofields);
    ...
    function setState (&$array)
    {
        if (isset ($array ['multirecord']))
		{
			...
			{
				throw new Exception ('RecordHandler "' .$this->m_name. '" is constructed with autofields and multirecord.  This is not compatible.');
			}
		}
	}
...
class DD_SqlRecordHandler extends DD_RecordHandler
{
	var			$m_connect;
	var			$m_conn;
	...
	function DD_SqlRecordHandler ($array, $extension = null)
	{
		$autofields = null;
	...
	function quickSelect ($connect, $sql, $tagParser = null)
	{
		...
		if (!isset ($conn))
		{
			die ("DD_SqlRecordHandler::quickSelect(): Cannot find connection string" . $connect);
		}
		if (isset ($tagParser))
		{
	...
	function query ($tagParser = null)
	{
		...
		DD_RecordHandler::query ($tagParser);
	}
	function nextRecord ()
	{
	...
	function execute ($sql, $tagParser = null)
	{
		...
		if (!isset ($this->m_conn))
		{
			die ("DD_SqlRecordHandler::execute(): Cannot find connection string" . $this->m_connect);
		}
		if (isset ($tagParser))
		{
