...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/htdocs/kpi/queryforlob.php,v 1.3 2012/08/28 21:14:33 secwind Exp $
*************************************************************************/
...
$selecthdr = <<<SQL
    select 'Performance Group: ' || name as name, name as itemname 
		,(select shortname from ai_kpi_export_prefix_map where id = {http.kpi_type}) as kpi_prefix
	from ai_kpi_group where id = {http.id}
SQL;
...
$page->makeField (array ('name'=>'xmllink', 'default'=>'kpi/queryforlob.php/{dd:hdr.kpi_prefix}_{dd:http.daterange}_{dd:%hdr.itemname}_by_{dd:user.tab_forlob}.xml?kpi_type={dd:http.kpi_type}&daterange={dd:http.daterange}&id={dd:http.id}&xml=true'));
$page->makeField (array ('name'=>'csvlink', 'default'=>'kpi/queryforlob.php/{dd:hdr.kpi_prefix}_{dd:http.daterange}_{dd:%hdr.itemname}_by_{dd:user.tab_forlob}.csv?kpi_type={dd:http.kpi_type}&daterange={dd:http.daterange}&id={dd:http.id}&csv=true'));

$page->makeField (array ('name'=>'list_filter','default'=>'kpi_group={dd:%http.id}&daterange={dd:http.daterange}&kpi_type={dd:http.kpi_type}&' . $page->get ('filter_arg')));
$page->makeField (array ('name'=>'total_filter','default'=>'kpi_group={dd:%http.id}&daterange={dd:http.daterange}&kpi_type={dd:http.kpi_type}'));
