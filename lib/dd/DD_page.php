...
** Author: Ben Balke
** $Header: /home/cvsroot/ai/lib/dd/DD_page.php,v 1.13 2012/08/28 22:00:19 secwind Exp $
*************************************************************************/
...
class DD_Page extends DD_FieldValueProvider
{
	...
	function DD_Page ($name, $noheaders = false, $userequest = false)
	{
		parent::DD_FieldValueProvider ($name);
		if (!$noheaders)
		{
	...
	function addRecordHandler ($rh)
	{
		$this->m_rhs [$rh->m_name] = $rh;
		$this->m_tp->addValueProvider ($rh);
	}
	...
	/** 
	 * process all the record handlers for a post.  The action is determined based on the 
	 ...
	 * records handlers are processed sequentially for all records in the order they are added to the page. 
	 * This means that different record handlers will not have access to the others fields values in sequence.
	 */
		...
		if (isset ($_POST['DD_records']))
		{
			...
			foreach($this->m_rhs as $name=>$rh)
			{
				if ($rh->m_multirecord)
				{
					...
					{
						$rh->setRecordNo ($r);
	    				$rh->setFieldsFromArray ($_POST, false);
						if (!$rh->isMultiRecordSkipped ())
						{
							switch ($type)
							{
							case DD_INSERT: $rh->insertRecord ($this->m_tp); break;
							case DD_UPDATE: $rh->updateRecord ($this->m_tp); break;
							case DD_DELETE: $rh->deleteRecord ($this->m_tp); break;
							}
						}
					}
				}
